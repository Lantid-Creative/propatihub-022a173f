  ...
  return (
    <AbsoluteFill style={{ padding: isP ? 60 : 70 }}>
      <div style={{ opacity: titleSp, transform: `translateY(${interpolate(titleSp, [0, 1], [30, 0])}px)` }}>
        <div style={{ fontFamily: heading, fontSize: isP ? 56 : 42, fontWeight: 800, color: C.gold }}>
          Built on Trust & Security
        </div>
      </div>

      <div style={{ display: 'flex', flexWrap: 'wrap', gap: isP ? 16 : 20, marginTop: isP ? 30 : 35 }}>
        {stats.map((s, i) => {
          const scale = spring({ frame: frame - (30 + i * 15), fps, config: { damping: 12 } });
          return (
            <div key={i} style={{
              transform: `scale(${scale})`,
              background: C.darkCard, borderRadius: 14, padding: isP ? '22px 26px' : '20px 28px',
              flex: isP ? '1 1 45%' : 1, textAlign: 'center', border: `1px solid ${C.green}30`,
            }}>
              <div style={{ fontSize: isP ? 32 : 24, marginBottom: 8 }}>{s.icon}</div>
              <div style={{ fontFamily: heading, fontSize: isP ? 42 : 32, fontWeight: 800, color: C.gold }}>{s.value}</div>
              <div style={{ fontFamily: body, fontSize: isP ? 18 : 11, color: C.gray, marginTop: 4 }}>{s.label}</div>
            </div>
          );
        })}
      </div>

      <div style={{ display: 'flex', flexDirection: isP ? 'column' : 'row', flexWrap: 'wrap', gap: isP ? 14 : 14, marginTop: isP ? 28 : 30 }}>
        {securityFeatures.map((f, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              padding: isP ? '16px 20px' : '12px 16px', borderLeft: `3px solid ${C.green}`,
            }}>
              <div style={{ fontFamily: heading, fontSize: isP ? 22 : 13, fontWeight: 700, color: C.white }}>{f.title}</div>
              <div style={{ fontFamily: body, fontSize: isP ? 18 : 11, color: C.gray, marginTop: 4 }}>{f.desc}</div>
            </div>
          );
        })}
      </div>
