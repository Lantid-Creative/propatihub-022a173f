...
export const Scene2Problem = () => {
  ...
  return (
    <AbsoluteFill style={{ padding: isP ? 60 : 80 }}>
      <div style={{ opacity: titleSpring, transform: `translateY(${interpolate(titleSpring, [0, 1], [30, 0])}px)` }}>
        <div style={{ fontFamily: heading, fontSize: isP ? 64 : 46, fontWeight: 800, color: C.gold }}>The Challenge</div>
        <div style={{ fontFamily: body, fontSize: isP ? 28 : 17, color: C.gray, marginTop: 10, maxWidth: isP ? 900 : 600 }}>
          Nigeria's real estate market is plagued by fraud, opacity, and broken trust.
        </div>
      </div>
      <div style={{ display: 'flex', flexDirection: isP ? 'column' : 'row', flexWrap: 'wrap', gap: isP ? 16 : 20, marginTop: isP ? 35 : 50 }}>
        {problems.map((p, i) => {
          const delay = 40 + i * 30;
          ...
          return (
            <div key={i} style={{
              ...
              padding: isP ? '18px 22px' : '18px 24px', width: isP ? '100%' : 'calc(50% - 10px)',
            }}>
              <div style={{ fontFamily: heading, fontSize: isP ? 26 : 17, fontWeight: 700, color: C.white, marginBottom: 4 }}>{p.title}</div>
              <div style={{ fontFamily: body, fontSize: isP ? 20 : 13, color: C.gray }}>{p.desc}</div>
            </div>
          );
        })}
      </div>
      <div style={{ display: 'flex', gap: isP ? 40 : 60, marginTop: isP ? 30 : 40, justifyContent: isP ? 'center' : 'flex-start' }}>
        {[{ v: "₦2.5T", l: "Market Value" }, { v: "17M", l: "Housing Deficit" }, { v: "60%", l: "Fake Listings" }].map((s, i) => (
          <div key={i} style={{ opacity: fade(frame, 200 + i * 20), textAlign: 'center' }}>
            <div style={{ fontFamily: heading, fontSize: isP ? 42 : 30, fontWeight: 800, color: C.gold }}>{s.v}</div>
            <div style={{ fontFamily: body, fontSize: isP ? 18 : 11, color: C.gray }}>{s.l}</div>
          </div>
        ))}
      </div>
    </AbsoluteFill>
